...
class FilterInputWidget(qt.QWidget):
    ...
    export_filtered_requested = (
        qt.Signal()
    )  # Signal emitted when "Export Filtered to HTML" is requested
    ...
    def __init__(self, parent=None):
        ...
        self.filter_input.setPlaceholderText(
            "Filter  [level: N|debug|info|warn|error] [host: ...] [process: ...] [thread: ...] [source: ...] [logger: ...] [message regex]"
        )
    ...
    def add_filter(self):
        ...
        if text:
            ...
            self.layout.insertWidget(
                self.layout.count() - 2, filter_tag
            )  # Insert before input and menu button
...
class HighlightDelegate(qt.QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        try:
            ...
            if parent_index.isValid():
                ...
                if highlight_type:
                    highlight_color = self._get_highlight_color(
                        highlight_type, is_child=True, palette=option.palette
                    )
            ...
            if highlight_type:
                highlight_color = self._get_highlight_color(
                    highlight_type, is_child=False, palette=option.palette
                )
                painter.fillRect(option.rect, highlight_color)
